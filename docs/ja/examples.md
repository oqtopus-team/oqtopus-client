...
`examples/` には Python スクリプトのサンプルがあります。

- `get_devices.py`
- `run_sampling.py` (`run_sampling`, `OqtopusSamplingJobResult`)
- `run_estimation.py` (`run_estimation`, `OqtopusEstimationJobResult`)
- `run_multi_manual.py` (`run_multi_manual`, `OqtopusMultiManualJobResult`)
- `run_sse_file.py` (`run_sse_file`, `OqtopusSseJobResult`)
- `run_sampling_quri_parts.py`
- `run_sampling_qiskit.py`
- `submit_jobs_parallel.py` (`OqtopusClient.submit_jobs` / `wait_for_jobs`)
- `run_job_generic.py` (`run_job`)
- `job_handle_lifecycle.py` (`OqtopusJobHandle` の主要メソッド)
...
- `init_client_from_env.py` (`from_env`, `set_api_token`, クライアント設定属性)
